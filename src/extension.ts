 ...
 * Deletes the temporary .tex file (passed as tempFilePath) and
 * all auxiliary files generated by pdflatex with the given jobname,
...
function cleanupTemporaryFiles(tempFilePath: string, buildDir: string, jobname: string) {
  ...
  try {
    ...
    for (const file of files) {
      if (path.basename(file, path.extname(file)) === jobname && path.extname(file) !== '.pdf') {
...
\\end{document}`;
  ...
  exec(command, { cwd: workspacePath }, (error, stdout, stderr) => {
    ...
    cleanupTemporaryFiles(tempFilePath, buildDir, "output");
  });
};
